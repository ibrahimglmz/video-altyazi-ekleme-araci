...
class OfflineSubtitleGenerator:
    ...
    async def process_with_multilingual_tts(self, src: Path, out_dir: Path, formats: List[str], 
                                          style: SubtitleStyle, language: str, include_ts: bool,
                                          tts_languages: List[str], embed_subtitles: bool = True,
                                          tts_engine: TTSEngine = TTSEngine.EDGE_TTS,
                                          original_audio_mix: float = 0.3) -> Dict[str, Any]:
        """Process video with multilingual TTS support"""
        
        # First, generate subtitles using existing method
        written = self.process_one(src, out_dir, formats, style, language, include_ts)
        
        # Find SRT file for TTS processing
        srt_path = None
        if 'srt' in written:
            srt_path = Path(written['srt'])
        elif 'srt' not in formats:
            # Generate SRT file for TTS processing
            self.progress_tracker.update(4, "Generating SRT for TTS processing...")
            cfg = style_preset(style)
            
            # Re-transcribe to get segments (this could be optimized)
            tmp_audio = out_dir / f"{src.stem}.tmp.wav"
            try:
                if is_video(src):
                    self.extract_audio(src, tmp_audio)
                    audio_path = tmp_audio
                else:
                    audio_path = src
                ...
                if segments:
                    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
                    srt_path = out_dir / f"{src.stem}_{timestamp}.srt"
                    self.write_srt(segments, cfg, srt_path)
                    written['srt'] = str(srt_path)
        ...
        if not srt_path or not srt_path.exists():
            raise RuntimeError("Could not generate SRT file for TTS processing")
        
        # Initialize TTS processor
        if not self.tts_processor:
            self.tts_processor = MultilingualTTSProcessor()
        
        self.progress_tracker.update(4, f"Generating TTS for {len(tts_languages)} languages...")
        
        # Process multilingual TTS
        tts_results = await self.tts_processor.process_multilingual_video(
            video_path=src,
            subtitle_path=srt_path,
        ...
        # Add TTS results to written files
        written.update({
            'tts_audio_files': [str(f) for f in tts_results['tts_audio']],
            'videos_with_tts': [str(f) for f in tts_results['videos_with_tts']],
            'videos_with_subtitles': [str(f) for f in tts_results['videos_with_subtitles']],
            'final_videos': [str(f) for f in tts_results['combined_videos']]
        })
        
        self.progress_tracker.update(5, "Multilingual processing complete!")
        return written
...
""")
    ...
    # TTS arguments
    p.add_argument('--tts-languages', 
                   help='Comma-separated list of languages for TTS (tr,en,fr,de,es,it,pt,ru,ja,ko,zh,ar)')
    p.add_argument('--tts-engine', default='edge_tts',
                   choices=['gtts', 'edge_tts'],
                   help='TTS engine to use (default: edge_tts)')
    p.add_argument('--no-embed-subtitles', action='store_true',
                   help='Do not embed subtitles in final videos')
    p.add_argument('--original-audio-mix', type=float, default=0.3,
                   help='Original audio volume ratio when mixing with TTS (0.0-1.0, default: 0.3)')
...
def main():
    ...
    # Parse TTS languages if provided
    tts_languages = []
    if args.tts_languages:
        tts_languages = [lang.strip().lower() for lang in args.tts_languages.split(',') if lang.strip()]
        
        # Validate TTS languages
